...
const _kAcceptedSuccessHttpCodees = [200, 201];
...
class ApiCall<T> {
  ...
  Stream<Resource<T>> _executeAsStream({
    bool allowEmptyResponseBody = false,
  }) async* {
    yield await execute(allowEmptyResponseBody: allowEmptyResponseBody);
  }

  Stream<Resource<T>> executeAsStream({
    bool allowEmptyResponseBody = false,
  }) {
    return _executeAsStream(allowEmptyResponseBody: allowEmptyResponseBody)
        .asBroadcastStream();
  }
  ...
  Future<Resource<T>> execute({bool allowEmptyResponseBody = false}) async {
    ...
    if (!_kAcceptedSuccessHttpCodees.contains(response.statusCode) &&
