...
// Define the background task only once to prevent multiple definitions
...
const defineLocationTask = () => {
  ...
  try {
    TaskManager.defineTask(LOCATION_TASK_NAME, async ({ data, error }) => {
      ...
      if (data) {
        try {
          ...
          if (location) {
            // Get the LocationService instance safely
            try {
              ...
              console.error('Error getting LocationService instance in background task:', instanceError);
              // Still try to update location even if instance access fails
              await updateUserLocationInFirestore(
                location.coords.latitude,
                location.coords.longitude
              );
    ...
    console.error('❌ Error defining background location task:', error);
  }
};

// Define the task immediately but with protection
// defineLocationTask();
...
export class LocationService {
  ...
  async startLocationTracking(): Promise<boolean> {
    try {
      ...
      let currentLocation;
      try {
        console.log('📍 Attempting to get current location with HIGH...');
        currentLocation = await Location.getCurrentPositionAsync({
          accuracy: Location.Accuracy.High,
          // timeInterval: 15000, // 15 second timeout for high accuracy
        });
        console.log('✅ Got current location (High):', {
          lat: currentLocation.coords.latitude,
          lng: currentLocation.coords.longitude,
          accuracy: currentLocation.coords.accuracy
        });
      } catch (locationError) {
        console.log('⚠️ Failed to get current location, trying with HIGH accuracy...');
        try {
          console.log('📍 Attempting to get current location with HIGH accuracy...');
          currentLocation = await Location.getCurrentPositionAsync({
            accuracy: Location.Accuracy.High,
            // timeInterval: 20000, // 20 second timeout for high accuracy
          });
          console.log('✅ Got current location (HIGH accuracy):', {
            lat: currentLocation.coords.latitude,
            lng: currentLocation.coords.longitude,
            accuracy: currentLocation.coords.accuracy
          });
      ...
      // Ensure task is defined before starting location updates
      defineLocationTask();
      console.log('✅ Background location task is ready');
